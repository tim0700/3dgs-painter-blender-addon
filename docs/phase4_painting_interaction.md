...
## 🏗️ 아키텍처: WorkSpaceTool 기반 페인팅

### Modal Operator vs WorkSpaceTool 비교

| 항목                 | Modal Operator (기존)                      | WorkSpaceTool (현재)            |
| -------------------- | ------------------------------------------ | ------------------------------- |
| **이벤트 처리**      | `modal()` 메서드에서 모든 이벤트 수동 처리 | Keymap 기반 자동 라우팅         |
| **UI 패널 상호작용** | 수동 region 체크 필요, 버그 발생 가능      | **자동 - UI 이벤트 자동 통과**  |
| **툴바 표시**        | 없음 (메뉴/핫키로만 접근)                  | 툴바에 아이콘 표시              |
| **Tool Header 설정** | N/A                                        | `draw_settings()` 메서드로 지원 |
| **브러시 프리뷰**    | Draw handler 직접 등록 필요                | Draw handler 사용 (동일)        |
| **구현 복잡도**      | 낮음                                       | 중간 (Tool + Operator 분리)     |
...
│  Toolbar (T)                                                │
│  ┌─────────────────┐                                        │
│  │ Gaussian Paint  │ ← GaussianPaintTool (WorkSpaceTool)    │
│  │    Tool         │   - bl_keymap: LMB → stroke operator   │
│  └────────┬────────┘   - draw_settings(): Tool Header UI    │
│           │                                                  │
│           │ keymap: LEFTMOUSE PRESS                          │
│           ▼                                                  │
│  ┌─────────────────────────────────────────────────────────┐│
│  │  THREEGDS_OT_GaussianPaintStroke (Modal Operator)       ││
│  │  - invoke(): 스트로크 시작, draw handler 등록            ││
│  │  - modal(): MOUSEMOVE → 스탬프 배치, RELEASE → 완료     ││
│  │  - _finish_stroke(): 변형 적용, cleanup                  ││
│  └─────────────────────────────────────────────────────────┘│
│                                                              │
│  ┌─────────────────┐      ┌──────────────────────────────┐  │
│  │  N Panel (UI)   │ ←──→ │  Scene Properties            │  │
│  │  - Brush Size   │      │  - npr_brush_size            │  │
│  │  - Opacity      │      │  - npr_brush_opacity         │  │
│  │  - Color        │      │  - npr_brush_color           │  │
│  └─────────────────┘      └──────────────────────────────┘  │
│         ↑                                                    │
│    자동 UI 상호작용 (WorkSpaceTool keymap 덕분)              │
└─────────────────────────────────────────────────────────────┘
```
...
```
src/
├── tools.py                    # GaussianPaintTool (WorkSpaceTool)
├── operators.py                # THREEGDS_OT_GaussianPaintStroke
├── viewport/
│   └── panels.py               # NPR_PT_PaintingPanel (브러시 설정 UI)
└── __init__.py                 # register_tools() / unregister_tools()
```

### 사용 방법

1. **툴바에서 도구 선택**: `T` 키로 툴바 열기 → "Gaussian Paint" 아이콘 클릭
2. **페인팅**: 3D 뷰포트에서 LMB 드래그
3. **설정 변경**: N 패널 → "3DGS Paint" → "Painting" 섹션에서 브러시 설정 조절
4. **도구 종료**: 다른 도구 선택 (예: Move, Rotate 등)

### 브러시 프리뷰 (Draw Handler)

```python
# operators.py - _register_brush_preview() 메서드

def draw_brush_cursor():
    """Draw circle at mouse position showing brush size."""
    radius = scene.npr_brush_size * 20  # 픽셀 단위 근사값
    color = (*scene.npr_brush_color, 0.5)  # RGBA

    gpu.state.blend_set('ALPHA')
    draw_circle_2d(mouse_pos, color, radius, segments=32)
